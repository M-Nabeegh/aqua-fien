...
// Use POSTGRES_URL_NON_POOLING if available, otherwise individual parameters
if (process.env.POSTGRES_URL_NON_POOLING) {
...
} else if (isSupabase && process.env.POSTGRES_URL) {
  // Use individual connection parameters with proper SSL configuration
  console.log('Using Supabase with SSL certificate')
  ...
  try {
    ...
    if (fs.existsSync(certPath)) {
      ...
      console.log('SSL certificate not found, using reject unauthorized false')
      sslConfig = { rejectUnauthorized: false }
    }
  } catch (error) {
    console.log('Error loading SSL certificate, falling back to reject unauthorized false:', error.message)
    sslConfig = { rejectUnauthorized: false }
  }
